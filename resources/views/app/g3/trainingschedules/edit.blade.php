		...
		<form id="add_form" method="post" role="form" action="">
			...
			<div class="box-body">
				...
				<div class="form-group">
					...
					<input type="" name="name" id="name" class="form-control" placeholder="{{__('general.Input a text...')}}" value="{{$training->name}}">
				</div>

				<div class="form-group">
					<label>{{__('general.Init')}}</label>
					<input type="" name="dt_ini" id="dt_ini" class="form-control" placeholder="DD/MM/AAAA HH:MM" value="{{preg_replace('#(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2})#', '$3/$2/$1 $4:$5', $training->dt_ini)}}">
				</div>
				<div class="form-group">
					<label>{{__('general.End')}}</label>
					<input type="" name="dt_end" id="dt_end" class="form-control" placeholder="DD/MM/AAAA HH:MM" value="{{preg_replace('#(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2})#', '$3/$2/$1 $4:$5', $training->dt_end)}}">
				</div>
				<div class="form-group">
					<label>URL</label>
					<input type="" name="url" id="url" class="form-control" placeholder="{{__('general.Input a text...')}}" value="{{$training->url}}">
				</div>
				...
				<div class="form-group">
					...
					<select  name="test_id" id="test_id" class="form-control select2-responsive" >
                        ...
                        <option value="{{$test->id}}" @if($test->id == $training->test_id) selected @endif>{{$test->name}}</option>
                        @endforeach
                    </select>
				</div>
                @can('master')
				...
				<div class="form-group">
					...
					<input type="number" name="vacancies" id="vacancies" class="form-control" placeholder="{{__('general.Input a text...')}}" value="{{$training->vacancies}}">
				</div>
