        ...
        <form id="add_form" method="post" role="form" action="">
            ...
            <div class="box-body">
                ...
                <div class="form-group">
                    ...
                    <input type="" name="name" class="form-control" placeholder="Digite um texto..." value="{{$test->name}}">
                </div>
                <div class="form-group">
                    <label>{{__('general.Minutes')}}</label>
                    <input type="number" name="minutes" class="form-control" placeholder="Digite um texto..." value="{{$test->minutes}}">
                </div>
                <div class="form-group">
                    <label>{{__('general.Questions')}}</label>
                    <input type="number" name="questions" class="form-control" placeholder="Digite um texto..." value="{{$test->questions}}">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select  name="document_id" class="select2-responsive form-control" >
                        ...
                        <option value="{{$document->id}}" @if($document->id == $test->document_id) selected @endif>{{$document->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select  name="company_id" class="select2-responsive form-control" >
                        ...
                        <option value="{{$company->id}}" @if($company->id == $test->company_id) selected @endif>{{$company->name}}</option>
                        @endforeach
                    </select>
                </div>
                @else
