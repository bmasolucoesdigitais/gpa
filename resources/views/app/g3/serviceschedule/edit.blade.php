...
<h1>{{ __('general.Scheduled services') }} - {{__('general.Company')}} - {{__('general.add')}}</h1>
@stop
		...
		<form id="add_form" method="post" role="form" action="" enctype="multipart/form-data">
			...
			<div class="box-body">
				...
				<div class="form-group">
					...
					<input type="" name="service" id="service" class="form-control" placeholder="{{__('general.Input a text...')}}" value="{{$service->service}}">
				</div>
					...
					<label>{{__('general.Employee')}}</label>
                        ...
                        <select   name="employees[]"  id="employees" class="form-control select2-responsive" multiple >
                            ...
                            @foreach($employees as $employee)
                                <option value="{{$employee->id}}" @if(in_array($employee->id, $employeesOnService)) selected @endif>{{$employee->name}} - {{$employee->cpf}}</option>
                            @endforeach
                        </select>
				</div>
				<div class="form-group">
